...
func _create_multimesh_colliders(
	...
	# --------------------------------------------------------
	# Convex Shape für jeden Mesh-Part nur EINMAL erzeugen.
	#
	# Die Shape befindet sich im lokalen Koordinatensystem
	# des jeweiligen Mesh-Parts.
	# --------------------------------------------------------

	var part_shapes: Array = []
	...
	for part in parts:
		...
		if shape == null:
			printerr(
				"[COLLIDER] Konnte Convex Shape nicht erzeugen: ",
				model_name,
				" / ",
				part["name"]
			)
	...
	# --------------------------------------------------------
	# Instanzen in Chunks aufteilen.
	#
	# Ein StaticBody enthält mehrere Instanzen.
	# Die Shapes selbst werden aber nicht zusammengefasst.
	# --------------------------------------------------------
	...
	while chunk_start < total_instances:
		...
		# ----------------------------------------------------
		# Jede Instanz bekommt ihre CollisionShapes.
		# ----------------------------------------------------
		...
		for instance_index in range(chunk_start, chunk_end):
			...
			for part_index in range(parts.size()):
				...
				# Genau dieselbe Transformation wie beim
				# entsprechenden MultiMesh-Part.
				collision_shape.transform = (
					instance_transform
					* internal_transform
				)

				# Die Shape selbst wird geteilt.
...
func _load_mesh_parts(
	...
	for mesh_instance in get_all_mesh_instances(
		dummy_root
	):
