...
@onready var text_bubble = $TextBubbleBackground
...
func _ready():
	...
	if(get_parent().play || get_parent().load_replay ||  (is_guard && !get_parent().record_guards)):
		var f = FileAccess.open("user://replay" + filepath_addon+".txt", FileAccess.READ)
		if(f != null):
			while !f.eof_reached():
				var line = f.get_line()
				if(line == ""):
				...
				if(parts[2] == "move" || parts[2] == "movethroughwindow"):
				...
				if(parts.size() >= 4):
					temp.append(parts[2])	
					if(parts[2] == "move" || parts[2] == "movethroughwindow"|| parts[2] == "use" ||parts[2] == "open" || parts[2] == "wait"):
						temp.append(parts[3])
					else:
		...
		var g = FileAccess.open("user://waiting_positions" + filepath_addon+".txt", FileAccess.READ)
		if(g != null):
			while !g.eof_reached():
				var line = g.get_line()
				if(line == ""):
...
func _physics_process(delta):
	...
	if(!get_parent().backwards):
		...
		for object in objects_in_sight_of_guard:
			if(object.get_parent().is_in_group("Actor")):
				if(!object.get_parent().is_guard):
					var space_state = get_world_3d().direct_space_state
								
					var query = PhysicsRayQueryParameters3D.create(mesh.global_position,object.get_parent().global_position)
					...
					if(result):
						if(result.collider.get_parent().is_in_group("Actor")):
							print(object.get_parent().name + " caught at " +str(get_parent().ticks) + " by " + unique_name)
							objects_in_sight_of_guard.remove_at(objects_in_sight_of_guard.find(object))
					
			if(object.get_parent().get_parent().is_in_group("Interactable")):
				var space_state = get_world_3d().direct_space_state
							
				var query = PhysicsRayQueryParameters3D.create(mesh.global_position,object.get_parent().get_parent().global_position)
				query.collide_with_areas = true
				query.set_collide_with_bodies(false)
				...
				if(result):	
					if(result.collider.get_parent().get_parent().is_in_group("Interactable")):
						if(result.collider.get_parent().get_parent().object_type == 0 || result.collider.get_parent().get_parent().object_type == 1):
							if(result.collider.get_parent().get_parent().anim_player.current_animation_position != 0):
								print("open door was found at " +str(get_parent().ticks) + " by " + unique_name)
								objects_in_sight_of_guard.remove_at(objects_in_sight_of_guard.find(object))							
		
		
	
	progress_bar.global_position = get_viewport().get_camera_3d().unproject_position(global_transform.origin)
	$TextBubbleBackground/TextBubble.global_position = get_viewport().get_camera_3d().unproject_position(global_transform.origin)
	$TextBubbleBackground.global_position = get_viewport().get_camera_3d().unproject_position(global_transform.origin)
	...
	$TextBubbleBackground/TextBubble.global_position += Vector2(-125,-250)
	$TextBubbleBackground.global_position += Vector2(-125,-250)
...
func _process(delta: float) -> void:
	if(Input.is_action_pressed("ui_cancel")):
		if(!get_parent().play && (!is_guard || get_parent().record_guards)):
			var f = FileAccess.open("user://replay" + filepath_addon+".txt", FileAccess.WRITE)
			#for e in replay:
			...
			var g = FileAccess.open("user://waiting_positions" + filepath_addon+".txt", FileAccess.WRITE)
...
func set_target_position(target_position: Vector3, record: bool):
	...
	$TextBubbleBackground.visible = false
	...
	if true:
		...
		#start with ticks and increment with each position
			...
			if(!is_zoomed_onto_object && mesh.global_position.distance_squared_to(comparision_position)>3):
			...
			if(get_parent().highlighted_object != null):
				if(selected_interaction == 1):
					...
					if(get_parent().highlighted_object.object_type == 6):
							...
							parts.append(get_parent().highlighted_object.name)
							replay.append(parts.duplicate())
							ticks_at_position+=1
							currentid +=1
					...
					if(get_parent().highlighted_object.object_type == 4):
						for j in range(pickup_time):
							...
							if(get_parent().highlighted_object.get_parent()is MeshInstance3D):
								parts.append(get_parent().highlighted_object.get_parent().get_parent().name+"/"+get_parent().highlighted_object.get_parent().name+"/"+get_parent().highlighted_object.name)
							else:	
								parts.append(get_parent().highlighted_object.get_parent().name+"/"+get_parent().highlighted_object.name)
							if(j < pickup_time -1):
								parts.append("none")
							else:
								parts.append("take")
				...
				if(selected_interaction == 2):
					if(get_parent().highlighted_object.object_type == 3):
						for j in range(3):
							parts.clear()
							if(j == 0):
								parts.append(ticks_at_position)
...
func _on_area_3d_area_entered(area: Area3D) -> void:
	...
	if(!get_parent().backwards):
		if(objects_in_sight_of_guard.find(area.get_parent())== -1):
			objects_in_sight_of_guard.append(area)
		
		if(area.get_parent().is_in_group("Actor")):
			if(!area.get_parent().is_guard):
				var space_state = get_world_3d().direct_space_state
							
				var query = PhysicsRayQueryParameters3D.create(mesh.global_position,area.get_parent().global_position)
				query.collide_with_areas = true
				query.set_collide_with_bodies(false)
				...
				if(result):
					if(result.collider.get_parent().is_in_group("Actor")):
						print(area.get_parent().name + " caught at " +str(get_parent().ticks) + " by " + unique_name)
				
		if(area.get_parent().get_parent().is_in_group("Interactable")):
			var space_state = get_world_3d().direct_space_state
...
func _on_area_3d_area_exited(area: Area3D) -> void:
	if(!get_parent().backwards):
		var index = objects_in_sight_of_guard.find(area)
		if(index != -1):
...
func show_text_bubble(text:String):
	$TextBubbleBackground.visible = true
	$TextBubbleBackground/TextBubble.text = text
